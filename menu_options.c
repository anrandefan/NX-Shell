	...
	if (in >= 0)
	{
		...
		if (out >= 0) // Opened file for writing
		{
			...
			while((b_read = read(in, buffer, chunksize)) > 0)
			{
				...
				if (displayAnim)
					ProgressBar_DisplayProgress(copymode == 1? "移动中" : "拷贝中", Utils_Basename(src), totalread, size);
			}
...
{
	int text_width = 0;
	TTF_SizeText(Roboto, "确定要继续吗？", &text_width, NULL);

	TTF_SizeText(Roboto, "是", &delete_confirm_width, &delete_confirm_height);
	TTF_SizeText(Roboto, "否", &delete_cancel_width, &delete_cancel_height);
...
void Menu_DisplayProperties(void)
{
	...
	SDL_DrawText(RENDERER, Roboto, 370, 133, config_dark_theme? TITLE_COLOUR_DARK : TITLE_COLOUR, "动作");
	...
	SDL_DrawTextf(RENDERER, Roboto, 390, 183, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "名称: %s", file->name);
	SDL_DrawTextf(RENDERER, Roboto, 390, 233, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "父级: %s", cwd);
	SDL_DrawTextf(RENDERER, Roboto, 390, 283, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "大小: %s", utils_size);

	if (file->isDir)
	{
		SDL_DrawText(RENDERER, Roboto, 390, 333, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "包含: ");
	...
	TTF_SizeText(Roboto, "确认", &properties_ok_width, &properties_ok_height);
...
void Menu_DisplayOptions(void)
{
	...
	TTF_SizeText(Roboto, "取消", &options_cancel_width, &options_cancel_height);
	...
	SDL_DrawText(RENDERER, Roboto, 385, 225, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "属性");
	SDL_DrawText(RENDERER, Roboto, 385, 327, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "重命名");
	SDL_DrawText(RENDERER, Roboto, 385, 429, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, cut_status? "粘贴" : "Move");
		
	SDL_DrawText(RENDERER, Roboto, 672, 225, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "新文件夹");
	SDL_DrawText(RENDERER, Roboto, 672, 327, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, copy_status? "粘贴" : "Copy");
	SDL_DrawText(RENDERER, Roboto, 672, 429, config_dark_theme? TEXT_MIN_COLOUR_DARK : TEXT_MIN_COLOUR_LIGHT, "删除");
}
